...
def predict_emotion(audio_path):
    ...
    st.write(f"MFCC mean: `{mfcc.mean():.4f}` | std: `{mfcc.std():.4f}`")

    mfcc_t = torch.tensor(mfcc, dtype=torch.float32).unsqueeze(0).to(device)

    with torch.no_grad():
        outputs = model(mfcc_t)
        st.write("Raw logits:", outputs.cpu().numpy().tolist())

        probs = torch.softmax(outputs, dim=1)
        st.write("All probs:", probs.cpu().numpy().tolist())
...
with tab1:
    ...
    if uploaded_file:
        ...
        temp_audio_path = "uploaded_temp.wav"
